import { useEffect, useState } from "react";
import { useNavigate, useParams } from "react-router-dom";
import { ArrowLeft, BookOpen, Clock, FileQuestion, Users } from "lucide-react";
import studentQuizApi from "../../api/studentQuizApi";
import styles from "./QuizDetailsStudent.module.css";

export default function QuizDetailsStudent() {
  const { id } = useParams();
  const navigate = useNavigate();
  const [quiz, setQuiz] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  useEffect(() => {
    const loadQuiz = async () => {
      try {
        setLoading(true);
        const quizData = await studentQuizApi.getQuizDetails(id);
        setQuiz(quizData);
      } catch (err) {
        setError(
          err?.response?.data?.message || err?.message ||
            "Impossible de charger les informations du quiz."
        );
      } finally {
        setLoading(false);
      }
    };

    loadQuiz();
  }, [id]);

  if (loading) {
    return <div className={styles.loading}>Chargement du quiz...</div>;
  }

  if (error) {
    return <div className={styles.errorBox}>{error}</div>;
  }

  return (
    <div className={styles.page}>
      <button className={styles.backBtn} onClick={() => navigate(-1)} aria-label="Retour">
        <ArrowLeft size={18} />
      </button>

      <div className={styles.card}>
        <div className={styles.topRow}>
          <div>
            <h1>{quiz?.titre || "Quiz sans titre"}</h1>
            <p>{quiz?.description || "Prévisualisation du quiz avant de le commencer."}</p>
          </div>
          <button
            className={styles.startBtn}
            onClick={() => navigate(`/student/quizzes/${id}/take`)}
          >
            Commencer le quiz
          </button>
        </div>

        <div className={styles.statsGrid}>
          <div>
            <BookOpen size={18} />
            <span>{quiz?.theme || "Thème non défini"}</span>
          </div>
          <div>
            <FileQuestion size={18} />
            <span>{quiz?.questionCount || 0} questions</span>
          </div>
          <div>
            <Clock size={18} />
            <span>{quiz?.timeLimit || 0} minutes</span>
          </div>
          <div>
            <Users size={18} />
            <span>{quiz?.totalStudentsAllowed || 0} étudiants</span>
          </div>
        </div>
      </div>
    </div>
  );
}
